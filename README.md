# 👋 Hi, I'm Muskan Chowdhary

💻 Aspiring Software Developer | Full Stack Enthusiast  
📍 Lucknow, India  

---

## 🚀 About Me
- 🎓 BCA Student (2023–2026)  
- 💡 Passionate about building real-world applications  
- ⚡ Focused on Full Stack Development (Java + React)  
- 📈 Interested in problem-solving & scalable systems  

---

## 🛠️ Tech Stack
**Languages:** Java, Python, JavaScript  
**Frontend:** React.js, HTML, CSS  
**Backend:** Core Java  
**Database:** MySQL  
**Tools:** Git, GitHub, VS Code  
**Other:** SEO, Generative AI, Prompt Engineering  

---

## 🚀 Featured Projects

### 🔹 Developers MVP (DevPulse)
A developer productivity tool that analyzes coding activity and provides actionable insights to improve workflow efficiency.  

### 🔹 Golf Slot Booking Platform
Full-stack web application for booking and managing golf slots with user dashboard and booking system.  

---

## 📊 GitHub Stats

![Stats](https://github-readme-stats.vercel.app/api?username=musk1903&show_icons=true&theme=tokyonight&count_private=true)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=musk1903&layout=compact&theme=tokyonight)
---

## 🔥 What I'm Currently Working On
- Building full-stack projects  
- Improving backend development skills  
- Learning system design basics  

---

## 📫 Connect With Me
- 💼 LinkedIn: https://linkedin.com/in/muskan-chowdhary-216361289  

---

⭐ *“Building projects that solve real problems.”*
